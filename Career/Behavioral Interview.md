# Resources:
- [Using the STAR method for your next behavioral interview (worksheet included) – Career Advising & Professional Development | MIT](https://capd.mit.edu/resources/the-star-method-for-behavioral-interviews/)

# STAR method: 1-2 minutes/ question
- Provide a sentence summary before starting
- *Situation (20%):* provide context for my examples
- *Task (10%):* my task, responsibilities, and goals
- *Action (60%):* actions I took to complete the task and goal. Highlight skills & character traits
- *Result (10%):* positive outcomes or results (**quantifiable** results). Lessons and key takeaways 
## Preparation:
- Identify technical and transferable skills
- Attention to usage and frequency of [action verb](https://cdn.uconnectlabs.com/wp-content/uploads/sites/123/2021/07/resume-action-verbs-capd.pdf)
- Brainstorm based on [Interviewing and the Job Offer](https://cdn.uconnectlabs.com/wp-content/uploads/sites/123/2022/06/STAR-Method-Worksheet-1.pdf), and my own [Behavioral Cheatsheet](https://docs.google.com/spreadsheets/d/1pardNI1sSZ2SYzh9Ok8cSOo72O5dZT9SmCf6AOp9kKw/edit?usp=sharing)

# Tips:
1. Don't lie. Tell based on your resume and cover letter
2. Focus on your contribution when talking about teamwork
3. Specific & clear examples
4. Relate back to the job posting

# Post interview:
- Record yourself and reflect
- Email interviewers within **24 hours** to thank for their time and emphasize your interest. Emailing each interviewer individually
- Reiterate why you're a great fit and mention noteworthy details from interview
# Personal Project:
- Challenges they overcame
- What made it interesting

# Workshop with Austen McDonald
## Structured evaluation
- *Signal areas:* scope, ownership, ambiguity, perseverance, conflict resolution, growth, communication, leadership
	- How to handle difficult things, break down task
	- Push through challenge
	- Conflict make us better, resolve in healthy way
	- growth mindset, learn from your experience
	- engage with others
	- leadership: amazon 16 leadership values
- *Preparation*: prepare core stories and practice telling them -> Go to the mode of talking about yourself
	- Practice pairing stories with the questions u might get
	- Do a lot of mock interview
- *Real time*: decode, select, deliver
	- Decode: understand the question
	- Select: pick the right stories
	- Deliver: practice in deliveries
## The Process
- *Step 1:* identify stories
	- Identify: title & setting -> context overview -> Action Inventory -> Results (measurable metric) & Impact (its overall affect on everything) -> Learning -> Signal area tag
	- 3-4 core stories: high-impact, challenging, leadership, learning exp, career transitions
- *Step 2:* extract meaningful details
	- CARL: Context **Action** (biggest) Results Learning
- *Step 3:* formulate stories based on CARL
- *Step 4:* big 3 questions
	- Tell me about yourself: set the stage
	- Favorite project: biggest scope, impact, personal involvement
	- Conflict story: highest stakes, deepest involvement, you were right
- *Step 5:* prepare meaningful questions
- *Step 6:* practice with yourself, AI

# Mock with CS VN:
- *Self-intro:*
	- Too broad, too much technical for a interview
	- Work well in the team: 1-2 sentence to shape personality & working style
	- Tailor to the role specifically
	- Work in projects similar to the company: high-level technical skills
	- Year and school -> Experience at ... -> Skills (1-2 sentence) -> Projects: ... -> Hobby
- *Why CS:* depends
	- Enthusiastic, projects that bring value to family & connects
	- Make it easy to empathize
- *Experience:*
	- No depth: discuss more about the thought process and procedure of the project
	- What didn't work well/ work well: expected more behavioral; how I adapt to new concepts, learn on the go
	- Conflict: should use STAR and CARL
	- See pros and cons in options
	- 70 Behavioral + 30 Technical
	- How to ask questions & How to learn
- *Soft skills:* elaborate things better, depth of the action, have an idea of what personality to show
	- Why company: look for internal info, company specific, conversational based
	- Split up: technical + innovation + both sided conversations
	- Read the room
- *Prepare questions:* at least 2 questions
	- Follow up on their response
	- Remember their team and talk about how long they stay in the company
- *TLDR:* tailor to STAR, and bullet point